{
	...
	plot_sub_graph_density <- function(track_subs, my_word, round, my_level){
		...
		plot_dens <- 
		  ...
		  ggplot() +
		  theme_minimal() +
			geom_histogram(aes(Date,p), stat = "identity") + 
		  #geom_smooth(aes(x = Date, y = p, color = Year), method = "loess", span = span) +
		  scale_x_date(date_breaks = "years", date_labels = "%Y", minor_breaks = "months") + 
			labs(y = "Proporiton of posts mentioning topic")
	...
	box_wc <- box(
		...
		plotOutput("plot_wc", height = "700px")
	)
	...
	box_dens <- box(
		side = "right", height = "auto",width = "auto",
		plotOutput("plot_dens", height = "700px")
	)
